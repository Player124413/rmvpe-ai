...
class RMVPE0PredictorV3:
    ...
    def __init__(self, model_path, device=None):
        ...
        self._trans = librosa.sequence.transition_local(N_CLASS, width=26, wrap=True)
    ...
    def decode(self, hidden, thred=0.03):
        ...
        window = 4
        expected_cents = np.zeros_like(path, dtype=np.float32)
        for t, idx in enumerate(path):
            start = max(idx - window, 0)
            end = min(idx + window + 1, probs.shape[0])

            local_probs = probs[start:end, t]
            weight_sum = local_probs.sum()
            if weight_sum > eps:
                local_cents = self.cents_mapping[start + 4:end + 4]
                expected_cents[t] = (local_probs * local_cents).sum() / weight_sum
            else:
                expected_cents[t] = self.cents_mapping[idx + 4]

        f0 = 10 * (2 ** (expected_cents / 1200))
    ...
    def smooth_f0_savgol_segments(
        ...
        for run in runs:
            if len(run) < poly + 2:
                continue
            # w = min(win, (len(run) | 1))
            max_w = len(run) if (len(run) % 2 == 1) else len(run) - 1
            w = min(win, max_w)
            logf = np.log2(f0[run])
            smoothed = savgol_filter(logf, w, poly, mode='mirror')
